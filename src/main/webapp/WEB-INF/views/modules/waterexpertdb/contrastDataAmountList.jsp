...
<head>
	...
	//	alert(type+"---type--");
		function getChart(type){
			...
			$("#searchForm").attr('action',"${ctx}/water/expertdb/getCompareChart?type="+type);
		...
		function setHref(type){
			...
			var link = "${ctx}/water/expertdb/getCompareChart?type="
	...
	<ul class="ul-form2">
		...
		<li>
			...
			<sys:treeselect id="compareDevice" name="deviceId" value="${compareDeviceId}" labelName="monitordevices.name" labelValue="${compareDeviceName}"
							title="对比监测点" url="/power/analysis/treeData?menu=water" cssClass="input-small" allowClear="true" notAllowSelectParent="true"/>
		</li>
		<li class="btns">
...
<script type="text/javascript">
	...
	if(option==0){
		...
		document.getElementById("contentTable").innerHTML="当前区间没有数据";
	}else{
