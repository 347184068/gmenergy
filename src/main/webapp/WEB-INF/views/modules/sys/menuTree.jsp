...
<body> --%>
	<div class="accordion" id="menu-${param.parentId}">
		...
		<div class="accordion-group accordion-body collapse ${not empty firstMenu && firstMenu ? 'in' : ''}">
			<div class="accordion-heading" >
				<a class="accordion-toggle"
				   data-parent="#menu-${param.parentId}" data-href="#collapse-${menu.id}"
				   <%--href="#collapse-${menu.id}"--%>
				   href="${fn:indexOf(menu.href, '://') eq -1 ? ctx : ''}${not empty menu.href ? menu.href : '/404'}"
				   target="mainFrame"  title="${menu.remarks}">
					<i class="icon-${not empty menu.icon ? menu.icon  : 'circle-arrow-right'} weui-list-menu1"
							style="font-size:50px;margin-left:30%;padding-right:70px"></i>

					<span class="weui-label1" style="padding-left:80%;padding-top:5px">${menu.name}</span></a>
			</div>
							...
							<c:if test="${menu2.parent.id eq menu.id&&menu2.isShow eq '1'}">
								<li>
									<a data-href=".menu3-${menu2.id}" href="${fn:indexOf(menu2.href, '://') eq -1 ? ctx : ''}${not empty menu2.href ? menu2.href : '/404'}" target="${not empty menu2.target ? menu2.target : 'mainFrame'}" >
										<i class="icon-${not empty menu2.icon ? menu2.icon : 'circle-arrow-right'} weui-list-menu2"></i>
										<span class="weui-label2">${menu2.name}</span></a>
