...
<head>
	...
	<script type="text/javascript">
		function getChart(){
			...
			//alert("---"+startTime+"---"+endTime+"--"+unitPrice);
			...
			$("#searchForm").attr('action',"${ctx}/energycharge/energyCharge/getAllDeviceChartInfo");
...
<div id="treeTable"  class="table table-striped table-bordered table-condensed">
...
<table id="contentTable" class="table table-striped table-bordered table-condensed">
	<thead>
	<tr>
		<th>序号</th><th>月份</th><th>总电量(kWh)</th><th>总电费(元)</th>
	</tr>
	</thead>
	<tbody>
	<c:choose>
	...
	<c:forEach items="${list}" var="energyCharge" varStatus="status">
		<tr>
			<td>${status.count}</td>
			<td>${energyCharge.indate}</td>
			<td>${energyCharge.sumElec}</td>
...
<script type="text/javascript">
	if(${empty list}){
		...
		//alert("option_elec-------"+JSON.stringify(option_elec));
		...
		//alert("option_charge-------"+JSON.stringify(option_charge));
